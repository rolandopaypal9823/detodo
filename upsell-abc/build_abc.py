...
/* CTA Instituto (al final de los recursos de cada módulo) */
.joincta{{margin-top:22px;border:1px solid var(--active-line);border-radius:16px;padding:28px 26px;text-align:center;position:relative;overflow:hidden;
  background:linear-gradient(180deg,rgba(255,102,2,.13),rgba(255,102,2,.02))}}
.joincta::before{{content:"";position:absolute;inset:-1px;border-radius:16px;background:radial-gradient(60% 80% at 50% 0%,var(--glow),transparent 70%);z-index:0;filter:blur(20px);opacity:.7;pointer-events:none}}
.joincta>*{{position:relative;z-index:1}}
.joincta__eye{{font-family:'JetBrains Mono',monospace;font-size:.62rem;letter-spacing:.18em;text-transform:uppercase;color:var(--orange);font-weight:500;margin-bottom:12px}}
.joincta__q{{font-family:'Montserrat';font-weight:800;color:var(--hueso);font-size:1.16rem;line-height:1.24;letter-spacing:-.01em;max-width:30ch;margin:0 auto 10px}}
.joincta__d{{color:var(--niebla);font-size:.97rem;line-height:1.6;max-width:48ch;margin:0 auto 22px}}
.joincta__d b{{color:var(--hueso);font-weight:700}}
.joincta__btn{{display:inline-flex;align-items:center;gap:10px;background:var(--orange);color:#fff;font-family:'Montserrat';font-weight:700;font-size:.98rem;padding:15px 30px;border-radius:999px;box-shadow:0 16px 34px -12px var(--glow);transition:transform .18s var(--ease),background .18s,box-shadow .3s}}
.joincta__btn:hover{{transform:translateY(-2px);background:var(--orange-hi);box-shadow:0 24px 46px -12px var(--glow)}}
.joincta__btn span{{transition:transform .2s var(--ease)}}
.joincta__btn:hover span{{transform:translateX(4px)}}
/* backdrop mobile */
...
(function(){{
  ...
  var INSTITUTE_CTA='<div class="joincta">'
    +'<div class="joincta__eye">El siguiente paso</div>'
    +'<div class="joincta__q">¿Ya sabés lo que tenés que hacer y no lo hacés?</div>'
    +'<div class="joincta__d">No necesitás más información. Necesitás un <b>sistema</b> y un <b>equipo de profesionales</b> que te acompañe.</div>'
    +'<a class="joincta__btn" href="https://nicolasfernandezmiranda.com/programa-instituto?utm_source=ebook&amp;utm_medium=bonos&amp;utm_content=abc" target="_blank" rel="noopener">Conocé cómo unirte al Instituto <span>&rarr;</span></a>'
    +'</div>';
  ...
  function renderResources(mi){{
    ...
    var html='';
    if(items.length){{
      html+='<div class="reslabel">📎 Recursos y entregables</div><div class="reslist">';
    ...
    box.innerHTML=html+INSTITUTE_CTA;
  }}
